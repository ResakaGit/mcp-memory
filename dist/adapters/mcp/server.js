...
import { errorToToolResult } from "../../domain/errors.js";
 ...
 */
export function registerMemoryModuleTools(server, ports) {
    ...
    const semConfig = { vectorDim: semanticConfig.vectorDim };
    {
        ...
        server.registerTool("get_recent_context", config, wrapToolHandler((args) => contextualApp.getRecentContext(contextual, args.agent_key, args.limit)));
    }
    {
        const config = {
        ...
        server.registerTool("append_contextual_memory", config, wrapToolHandler((args) => contextualApp.appendContextualMemory(contextual, args.agent_key, args.new_entry)));
    }
    {
        const config = {
        ...
        server.registerTool("get_all_context", config, wrapToolHandler((args) => contextualApp.getAllContext(contextual, args.agent_key)));
    }
    {
        const config = {
        ...
        server.registerTool("append_semantic_memory", config, wrapToolHandler((args) => semanticApp.appendSemanticMemory(semantic, semConfig, args.agent_key, args.embedding, args.content, args.metadata)));
    }
    {
        const config = {
        ...
        server.registerTool("search_semantic_memory", config, wrapToolHandler((args) => semanticApp.searchSemanticMemory(semantic, semConfig, args.agent_key, args.embedding, args.top_k, args.score_threshold)));
    }
    {
        const config = {
        ...
        server.registerTool("get_memory_stats", config, wrapToolHandler((args) => contextualApp.getMemoryStats(contextual, args.agent_key)));
    }
    {
        const config = {
        ...
        server.registerTool("rollup_memory_segment", config, wrapToolHandler((args) => contextualApp.rollupMemorySegment(contextual, args.agent_key, args.keep_last, args.summary_entry)));
    }
    {
        const config = {
        ...
        server.registerTool("set_scratchpad_entry", config, wrapToolHandler((args) => scratchpadApp.setScratchpadEntry(scratchpad, args.agent_key, args.name, args.value, args.ttl_seconds)));
    }
    {
        const config = {
        ...
        server.registerTool("get_scratchpad_entry", config, wrapToolHandler((args) => scratchpadApp.getScratchpadEntry(scratchpad, args.agent_key, args.name)));
    }
    {
        const config = {
        ...
        server.registerTool("clear_scratchpad_entry", config, wrapToolHandler((args) => scratchpadApp.clearScratchpadEntry(scratchpad, args.agent_key, args.name)));
    }
    {
        const config = {
        ...
        server.registerTool("publish_interagent_message", config, wrapToolHandler((args) => eventsApp.publishInteragentMessage(events, buildChannelName(args.channel), args.sender_agent_key, args.payload)));
    }
    {
        const config = {
        ...
        server.registerTool("get_channel_log", config, wrapToolHandler((args) => eventsApp.getChannelLog(events, buildChannelName(args.channel), args.limit)));
    }
}
...
export async function startServer(ports) {
    const server = new McpServer({ name: memoryMcpModule.name, version: memoryMcpModule.version }, { capabilities: { tools: { listChanged: false } } });
