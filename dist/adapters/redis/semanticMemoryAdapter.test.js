...
describe("appendSemanticMemory", () => {
    it("rechaza embedding con dimensión incorrecta", async () => {
        ...
        const result = await app.appendSemanticMemory(port, { vectorDim: VECTOR_DIM }, input.agent_key, input.embedding, input.content, input.metadata);
    ...
    it("guarda entrada y llama ensureIndex y append", async () => {
        ...
        const result = await app.appendSemanticMemory(port, { vectorDim: VECTOR_DIM }, "agent:arq_1", embedding, "Resolví un problema de base de datos");
...
describe("searchSemanticMemory", () => {
    it("rechaza embedding con dimensión incorrecta", async () => {
        const redis = createMockRedis();
        const port = createSemanticMemoryAdapter(redis);
        const result = await app.searchSemanticMemory(port, { vectorDim: VECTOR_DIM }, "agent:arq_1", Array(VECTOR_DIM - 1).fill(0), 5);
        expect(result.isError).toBe(true);
    });
    it("devuelve resultados formateados cuando el mock retorna hits", async () => {
        const redis = createMockRedis();
        const port = createSemanticMemoryAdapter(redis);
        const result = await app.searchSemanticMemory(port, { vectorDim: VECTOR_DIM }, "agent:arq_1", Array(VECTOR_DIM).fill(0.1), 3);
        expect("isError" in result ? result.isError : undefined).toBeFalsy();
        expect(result.content[0].text).toContain("Recuerdo de prueba");
    });
});
//# sourceMappingURL=semanticMemoryAdapter.test.js.map