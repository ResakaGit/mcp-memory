...
function createMockRedis(): RedisClient {
  ...
  const resolveIndex = (arr: string[], i: number) =>
    i < 0 ? Math.max(0, arr.length + i) : i;
  return {
...
describe("get_memory_stats y rollup_memory_segment", () => {
  ...
  it("rollupMemorySegment reemplaza por resumen + últimos N", async () => {
    ...
    const port = createContextualMemoryAdapter(redis);
    const result = await app.rollupMemorySegment(
      port,
      "agent:1",
      2,
      "[RESUMEN HISTÓRICO] Trabajo previo completado."
    );
...
describe("schemas", () => {
  ...
  it("appendContextualMemorySchema rechaza new_entry vacío", () => {
    expect(() =>
      schemas.appendContextualMemorySchema.parse({ agent_key: "agent:x", new_entry: "" })
    ).toThrow();
  });
});
